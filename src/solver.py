...
def rlasso(x, y, colnames=None, post=True, intercept=True, model=True, 
           homoskedastic=False, X_dependent_lambda=False, lambda_start=None, 
           c=1.1, gamma=None, numSim=5000, numIter=15, tol=10**(-5), 
           threshold=-np.inf, par=True, corecap=np.inf, fix_seed=True):
    ...
    if ~(post or homoskedastic or X_dependent_lambda) and (lambda_start == None) and (c == 1.1) and (gamma == .1 / np.log(n)):
        c = .5
    
    est = None

    if intercept:
        meanx = cvec(x.mean(axis = 0))
        mu = y.mean()
    else:
        meanx = np.zeros((n, 1))
        mu = 0
    
    x = x - np.ones((n, 1)) @ meanx.T
    y = y - mu
    ...
    pen = lambdaCalculation(homoskedastic = False, X_dependent_lambda = False, 
                            lambda_start = None, c = 1.1, gamma = 0.1, 
                            numSim = 5000, y = None, x = None, par = True, 
    ...
    while mm <= numIter:
        if (mm == 1) and post:
            coefTemp = (
                LassoShooting_fit(x, y, lmbda / 2, XX = XX, Xy = Xy)["coefficients"]
            )
        else:
            coefTemp = (
                LassoShooting_fit(x, y, lmbda, XX = XX, Xy = Xy)["coefficients"]
            )
